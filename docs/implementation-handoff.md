...
后端服务基本仍只有骨架与健康接口，外加四处例外：case-service 的 `POST /api/v1/auth/login`（演示账号换 JWT）、`GET /api/v1/orders`、`GET /api/v1/orders/{order_id}`、`POST /api/v1/cases`（建单：`Idempotency-Key` 幂等、同 line 单活跃 case、行归属由 Commerce 判定）、`GET /api/v1/cases/{case_id}`（工单视图 + 轨迹；跨主体 404）与 `POST /api/v1/cases/{case_id}/evidence`（材料追加：只追加不覆盖、revision 由服务端决定、来源决定谁可提交、已终结则 409）与 、`POST /api/v1/cases/{case_id}/cancel`（消费前取消：终态、释放活跃 slot、不可重复取消）与 `GET /api/v1/cases/{case_id}/events`（SSE 轨迹流：先重放后推送、Last-Event-ID 续传、终态收流）；政策由受控命令导入（`--resolveflow.policy.import-dir`，不可变版本 + 按支付时间选择窗口），只读面 `GET /internal/v1/policies/{bundle_id}` 供 Agent 取正文，`GET /internal/v1/cases/{case_id}/policy-manifest` 返回按支付时间选中并钉在工单上的版本；作用域一律由已校验主体推导，内部调 commerce-service 的 `GET /internal/v1/order-lines`（列表）与 `GET /internal/v1/order-lines/{line_id}/context`（单行 + 该订单 ledger 的 `refunded_amount`/`reserved_refund_amount`，Java 重算金额的权威来源），两者都 service token 专用，数据来自 `commerce_db` 的迁移与种子。回调面 `POST /internal/v1/cases/{case_id}/agent-callbacks` 已实现收件箱与投递语义（`callback_id` 主键即「最多应用一次」、逐字重投 DUPLICATE、旧 revision 或人工接管后 STALE、同 id 换内容 409、STARTED/QUESTION/FAILED 各自的状态与轨迹后果），**PROPOSAL 已可接受**：Java 按工单钉住的政策版本核对引用（`bundle_id`/`version`/`rule_id`/`chunk_id` 与重算的 `content_hash`）并从 Commerce 的行上下文重算金额，结果以 `case_proposal` 存下 `VALIDATED` 或带原因的 `REJECTED`，工单转入 `PENDING_REVIEW`，方案随工单视图返回（旧 revision 显示为 `STALE`）；结构或绑定错误（`case_id`/`input_revision`/未知动作/非 REFUND 带金额等）仍是 422。case-service 需要 `case_db`（Flyway 迁移 + `case_svc` 账户）。核心其余 15 条路由（28 − 13）、退款闭环、Agent 调查/政策检索、恢复与回放、实验与主张都未实现；observability 容器尚不存在。`contracts/core` 里的目标协议不代表已在运行。
