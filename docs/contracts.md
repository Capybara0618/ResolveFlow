...
> v1.2迁移说明：本文件保留T02已经验证的v1接口全集，当前仅作为兼容基线，不再定义核心必做范围。核心实现以[核心范围](core-scope.md)和[核心协议目标](core-contracts.md)为准，C00建立独立核心Schema/OpenAPI与覆盖测试。下文旧T任务、补发/权益接口和“必须全部实现”措辞仅描述历史方案，不继续执行。保留此文件是因为既有契约测试直接解析其路由；不能删除测试来通过瘦身迁移。
