...
- 实际记录：
  ...
  - 未执行：`verify -Suite format` 与 `-Suite all-offline` 本轮未在修正后完整重跑（修正后已完整重跑 → **PASS**，含 Java Spotless 与 web typecheck，报告 `reports/verify/20260919-095311-format.txt`）；C00.2c-2（核心正反 fixture 与 core_corpus）、C00.2c-3（Java/Pydantic 核心 DTO）、C00.3（core profile 启动与 smoke 选择）未开始；core profile 的服务启动尚未验证。
