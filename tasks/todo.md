...
- 实际记录：
  ...
  - 未执行：C01.2b（commerce-service 的 `demo` 迁移与订单/支付快照种子、`GET /internal/v1/order-lines` 与 `LineContext` 读接口、MySQL 集成测试与「账户不跨库读取」测试）与 C01.2c（Case 公共订单视图与跨主体 404）均未开始；**新增的 commerce 路由目前仍未实现**，README 里已如实标注。
