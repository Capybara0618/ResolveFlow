...
- 实际记录：
  ...
  - 未执行：C01.2b-2（`GET /internal/v1/order-lines` 的 HTTP 路由与 service token 校验、`LineContext` 读接口、以及用 `infra/mysql/init/01-databases.sql` 验证「账户不跨库读取」的集成测试）与 C01.2c（Case 公共订单视图与跨主体 404）均未开始；**新增的 commerce 路由目前仍不可调用**。
