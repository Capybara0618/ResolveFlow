...
28 条核心路由里真正实现的目前是 **12 条**：case-service 的 `POST /api/v1/auth/login`、`GET /api/v1/orders`、`GET /api/v1/orders/{order_id}`、`POST /api/v1/cases`（建单幂等与同 line 单活跃 case）、`GET /api/v1/cases/{case_id}`（工单视图与轨迹）、`POST /api/v1/cases/{case_id}/evidence`（材料追加、revision 由服务端决定、来源决定谁可提交）与 `POST /api/v1/cases/{case_id}/cancel`（消费前取消：终态、释放活跃 slot、不可重复取消）与 `GET /api/v1/cases/{case_id}/events`（SSE：先重放后推送、Last-Event-ID 续传、终态收流、token 只在 header），`GET /internal/v1/policies/{bundle_id}` 与 `GET /internal/v1/cases/{case_id}/policy-manifest`（service token 专用：前者是 Agent 引用的政策正文，Java 据此核对引用而不是核对模型摘要；后者是工单被钉住的政策版本，按支付时间选择、开单事务内写入），以及 commerce-service 的 `GET /internal/v1/order-lines`（service token 专用，供前两者使用）与`GET /internal/v1/order-lines/{line_id}/context`（service token 专用：Java 重算退款金额的权威读取，带 `refunded_amount`/`reserved_refund_amount`；列表面不回显归属与已动金额，context 必须回显，因为调用方要用它确认该行属于自己正在复核的工单）。另外，`POST /internal/v1/cases/{case_id}/agent-callbacks` 的收件箱与投递语义（ACCEPTED/DUPLICATE/STALE、STARTED/QUESTION/FAILED）已实现，但 **PROPOSAL 仍会以 422 拒绝**——它要等 C03.2b-2 的 Java 重检（引用核对与金额重算）；因此这条路由**不计入**上面 12 条，避免把「三种 kind 可用」说成「路由已实现」。它按核心 OpenAPI 的 `LoginRequest`/`LoginResponse` 出入参，登录不需要 token，失败走统一错误体（401 `UNAUTHENTICATED` 对"口令错"与"账号不存在"只有同一条消息，400 `INVALID_ARGUMENT` 拒绝未知字段）。
