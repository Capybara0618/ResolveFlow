...
28 条核心路由里真正实现的目前是 **9 条**：case-service 的 `POST /api/v1/auth/login`、`GET /api/v1/orders`、`GET /api/v1/orders/{order_id}`、`POST /api/v1/cases`（建单幂等与同 line 单活跃 case）、`GET /api/v1/cases/{case_id}`（工单视图与轨迹）、`POST /api/v1/cases/{case_id}/evidence`（材料追加、revision 由服务端决定、来源决定谁可提交）与 `POST /api/v1/cases/{case_id}/cancel`（消费前取消：终态、释放活跃 slot、不可重复取消）与 `GET /api/v1/cases/{case_id}/events`（SSE：先重放后推送、Last-Event-ID 续传、终态收流、token 只在 header），以及 commerce-service 的 `GET /internal/v1/order-lines`（service token 专用，供前两者使用）。它按核心 OpenAPI 的 `LoginRequest`/`LoginResponse` 出入参，登录不需要 token，失败走统一错误体（401 `UNAUTHENTICATED` 对"口令错"与"账号不存在"只有同一条消息，400 `INVALID_ARGUMENT` 拒绝未知字段）。
